...
var (
	systrayReady  func()
	systrayExit   func()
	menuLClick    *MenuItem                   // 왼쪽 클릭시 동작하는 메뉴 (메뉴 미시현)
	menuItems     = make(map[int32]*MenuItem) // 등록된 메뉴 (오른쪽 클릭 시 시현)
...
func AddLeftClickMenuItem() *MenuItem {
	item := newMenuItem("lclick", "", nil)
	menuLClick = item
	return item
}
...
func systrayIconClicked() {
	...
	select {
	case menuLClick.ClickedCh <- struct{}{}:
	// in case no one waiting for the channel
	default:
	}
}
